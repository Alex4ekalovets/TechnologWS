...
logging.basicConfig(
    # filename=os.path.join("cdw_reader.log"),
    # filemode='a',
...
class CDWReader:
    ...
    allowed_titles = ("Наименование", "Материал", "Количество", "Длина", "Чертеж")
    _prohibited_texts = ("Детали", "Сборочные единицы", "Изготовить", "@", "Документация", "Стандартные изделия")

    def __init__(self, files_paths):
        self._module = gencache.EnsureModule(
            typelibCLSID="{69AC2981-37C0-4379-84FD-5DD2F3C0A520}",
            lcid=0,
            major=1,
            minor=0
        )
        self._api = self._module.IKompasAPIObject(
            Dispatch("Kompas.Application.7")._oleobj_.QueryInterface(
                self._module.IKompasAPIObject.CLSID,
                pythoncom.IID_IDispatch
            )
        )
        self._app = self._api.Application  # Получаем основной интерфейс
        self._const = gencache.EnsureModule(
    ...
    # def _get_all_data_with_draw(self):
    #     """
    ...
    def _get_all_data(self):
        ...
        specification = list()
        for file_path in self._files_paths:
            self.filename = os.path.split(file_path)[-1].replace(".cdw", "")
            document = self._open_document(file_path)
            try:
                for func in (self._parse_specifications, self._parse_tables):
                    data = func(document)
                    specification.extend(data)
            except Exception as ex:
                logging.exception(ex)
        self._app.HideMessage = self._const.ksShowMessage
        self._close_all_documents()
    ...
    def _parse_specifications(self, document):
        ...
        self._add_draw(specification)
        return specification

    def _add_draw(self, specification):
        for i, row in enumerate(specification):
            specification[i]['Чертеж'] = self.filename
